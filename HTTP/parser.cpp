...
HTTP_CODE Parser::start_parse(int fd, char *buf, int read_idx, int& parser_idx, int& line_start_idx)  {
    ...
    HTTP_CODE retcode;
    while(user[fd].check_state == CHECK_STATE_CONTENT && linestatus == LINE_OK || (linestatus = parse_line(buf, parser_idx, read_idx)) == LINE_OK  )
    {
        buf_ptr = buf + line_start_idx;
        line_start_idx = parser_idx;
        switch (user[fd].check_state)
        {
            case CHECK_STATE_REQUESTLINE:
            {
                retcode = parse_requestline(fd, buf_ptr);
                if ( retcode == REQUEST_KPON ) {
                    user[fd].check_state = CHECK_STATE_HEADER;
                }  
                else {
                    return REQUEST_BAD;
                }
            ...
            case CHECK_STATE_HEADER: 
            {
                ...
                if ( retcode == REQUEST_GET ) {
                    user[fd].method = REQUEST_GET;  //修改method
                    return REQUEST_GET; 
                }
                if ( retcode == REQUEST_POST ) {
                    user[fd].method = REQUEST_POST; //修改method
                    user[fd].check_state = CHECK_STATE_CONTENT;
                }
